...
export default new BaseFunction({
    ...
    code: async function(d, [property, memberID = d.ctx?.user?.id, guildID = d.ctx?.guild?.id]) {
        ...
        const types = Object.keys(member)
        if (!types.includes(property)) throw new d.error(d, 'invalid', 'Property', d.function?.name!)

        return typeof member[property] === 'string' ? member[property] : typeof member[property] === 'number' ? member[property].toString() : inspect(member[property])
    }
})